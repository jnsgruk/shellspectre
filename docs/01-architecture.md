...
- Contains: `ExecEvent`, `IoEvent`, `ExitEvent`, `TaskFieldOffsets`, `EventType`, the versioned `EventHeader`, and the shared SQLite schema
- Single `lib.rs`
...
- FD filter: only file descriptors 0, 1, 2 (stdin/stdout/stderr)
...
|--------|---------------|
| `main.rs` | CLI entry, eBPF program loading, async ring buffer consumption loop |
...
1. **Tracepoint fire.** eBPF programs attached to syscall tracepoints (`sys_enter_execve`, `sys_exit_execve`, `sys_enter_write`, `sys_exit_write`, `sys_enter_read`, `sys_exit_read`, `sys_enter_exit_group`) execute on each matching syscall. Programs read `task_struct` fields (pid, tgid, comm, tty_nr, ppid) using BTF-resolved offsets passed in via the `OFFSETS` map.

2. **Scratch arrays.** `ExecEvent` (~5.5KB) exceeds the 512B BPF stack limit. The exec path uses a `PerCpuArray` scratch entry (`EXEC_SCRATCH`) to build events before moving them into maps or the ring buffer.
...
2. **Ancestor inheritance.** On execve, if the process's `comm` matches a configured ancestor name (e.g., `sshd`, `bash`), a new session is created. Otherwise, the process inherits its parent's `session_id` from the correlator's `pid -> session_id` map.
