...
### shspectr-common/ — Single File

`lib.rs`: `#[repr(C)]` event structs, `EventType` enum, `TaskFieldOffsets`. Feature-gated serde. Shared between kernel and userspace — must stay `#![no_std]`.
...
```
shspectr/src/
├── main.rs           # CLI parsing, eBPF loading, ring buffer loop
