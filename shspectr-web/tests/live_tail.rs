...
fn insert_test_exec(conn: &rusqlite::Connection, session_id: &str, pid: u32, comm: &str) {
    ...
    conn.execute(
        "INSERT INTO events \
         ...
         VALUES (?1, 0, datetime('now'), ?2, ?2, 1, 1000, 1000, 1000, ?3, ?4, '[]', 0)",
...
async fn live_endpoint_with_filter_only_streams_matching() {
    ...
    match body {
        Ok(text) => {
            assert!(
                text.contains("bash"),
                "should contain bash event, got: {text}"
            );
            assert!(
                text.contains("datastar-patch-elements"),
